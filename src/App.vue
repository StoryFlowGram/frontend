<template>
  <div class="h-screen overflow-hidden relative">
    ...
    >
      ...
      <p class="text-gray-500 text-sm mt-8 text-center max-w-sm">
        v.0.0.1InDev
      </p>
    </div>
...
const isLoading = ref(true) // По умолчанию true, пока проверяем сессию
...
const currentViewComponent = computed(() => {
  return viewComponents[currentView.value] || LibraryView
})
...
// Загрузка библиотеки (используется как проверка авторизации)
const loadUserLibrary = async () => {
  try {
    ...
    // Если запрос прошел (200 OK) - значит мы авторизованы
    userLibrary.value = response.data.items || []
    isAuthenticated.value = true 
  } catch (error) {
    // 401 - это штатная ситуация "Пользователь не вошел"
...
const loginWithTelegram = async () => {
  try {
    ...
    // Проверка для продакшена (можно убрать для тестов в браузере)
    // if (!user) {
    //   alert('Запустіть додаток через телеграм!')
    //   isLoading.value = false
    //   return
    // }

    const payload = {
      telegram_id: user?.id || 123456789, // Фейк ID для браузера
      first_name: user?.first_name || 'BrowserUser',
      last_name: user?.last_name || null,
      username: user?.username || null
    }

    const response = await axios.post(`${API_URL}/auth/telegram`, payload, {
      withCredentials: true 
    })
    ...
    errorMessage.value = 'Помилка входу через Telegram'
  } finally {
    isLoading.value = false
  }
}

// Вход через Google (открытие окна)
...
const handleGoogleCode = async (code) => {
  ...
  try {
    ...
    errorMessage.value = error.response?.data?.detail || "Помилка авторизації Google"
  } finally {
    isLoading.value = false
  }
}

// Обработка кода от Google (Привязка в профиле)
const handleLinkGoogle = async (code) => {
  isLoading.value = true
  try {
    await axios.post(`${API_URL}/auth/link/google`, { 
      code: code,
      redirect_uri: window.location.origin 
    }, {
      withCredentials: true,
    ...
    showNotification(error.response?.data?.detail || "Помилка прив'язки", 'error')
  } finally {
    isLoading.value = false
...
const addToLibrary = async (book) => {
  try {
    if (!userLibrary.value.some(b => b.id === book.id)) {
...
const navigateToView = (viewName) => { currentView.value = viewName }
const openBook = (book) => { selectedBook.value = book }
const openBookDetail = (book) => { selectedBookForDetail.value = book }

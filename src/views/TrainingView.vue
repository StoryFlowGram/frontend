  ...
  <div class="min-h-screen bg-white">
    ...
    <div v-else-if="mode === 'flashcards'" class="px-6 py-12">
      <div class="max-w-md mx-auto">
        <div class="relative h-96 cursor-pointer perspective-1000" @click="flipCard">
          <div
            :class="[
          ...
          >
            ...
            >
              ...
              <div v-if="currentWord.context" class="bg-gray-50 rounded-2xl p-4 w-full">
                <p class="text-sm text-gray-600 italic text-center">"{{ currentWord.context }}"</p>
              </div>
            </div>
          </div>
        </div>
...
const sendReview = async (cardId, quality) => {
  try {
    ...
    await api.patch('/learning/review', {
      card_id: cardId,
      quality: quality
    })
  } catch (error) {
...
const generateOptions = () => {
  ...
  const allTranslations = props.cards.map((c) => c.translation).filter((t) => t !== correct)

<template>
  <div class="min-h-screen bg-white pb-24">
    ...
    <div class="px-6 py-6 space-y-8">
      ...
      <section v-if="continueReadingBooks.length > 0">
        ...
        <div class="flex gap-4 overflow-x-auto pb-4 -mx-6 px-6 snap-x snap-mandatory scrollbar-hide">
          ...
          >
            <div class="bg-gray-900 rounded-2xl overflow-hidden hover:shadow-xl transition-shadow duration-200 cursor-pointer h-full flex flex-col relative">
              ...
              <div class="p-4 flex flex-col flex-1">
                <div>
                  <h3 class="font-semibold text-white line-clamp-2 leading-tight">{{ book.title }}</h3>
      ...
      <section>
        ...
        <div v-else class="grid grid-cols-2 gap-4"> <div
          ...
          >
            <div class="aspect-[2/3] bg-gray-200 relative">
              ...
              <div v-if="book.isAdded" class="absolute top-2 right-2 bg-green-500 text-white p-1.5 rounded-full shadow-lg z-10">
                <Check class="w-3 h-3" />
              </div>
            </div>

            <div class="p-3 flex flex-col flex-1">
              <h3 class="font-bold text-black text-sm line-clamp-2 leading-snug mb-1">{{ book.title }}</h3>
              ...
              <div class="mt-auto pt-2 flex items-center justify-between">
                ...
                >
                  ...
                  <span v-else class="animate-spin rounded-full h-3 w-3 border-b-2 border-white"></span>
                </button>
                ...
                >
                  ...
                  <span v-else class="animate-spin rounded-full h-3 w-3 border-b-2 border-red-500"></span>
                </button>
              </div>
            </div>
          </div>
        ...
        <div v-if="!isLoading && filteredBooks.length === 0" class="flex flex-col items-center justify-center py-12 text-gray-500">
    ...
    <Transition name="toast">
      <div v-if="toast.show" :class="['fixed top-20 left-1/2 transform -translate-x-1/2 px-6 py-3 rounded-full shadow-2xl z-50 flex items-center gap-2 text-white text-sm font-medium', toast.type === 'error' ? 'bg-red-500' : 'bg-black']">
...
const emit = defineEmits(['open-book-detail', 'open-reader'])
...
const fetchAllData = async () => {
  try {
    ...
    const [booksResponse, readerResponse] = await Promise.all([
      axios.get(`${API_URL}/book/list`, { withCredentials: true }), 
    ...
    booksData.value = allBooks.map(book => {
      // Find this book in user's library
      const libraryEntry = userLibrary.find(lb => lb.book_id === book.id)
      
      let progressPercent = 0
      if (libraryEntry && libraryEntry.overall_progress) {
...
const continueReadingBooks = computed(() => {
  ...
  return booksData.value.filter(book => book.isAdded)
})
...
const addToLibrary = async (book) => {
  ...
  try {
    const response = await axios.post(`${API_URL}/reader/`, 
      { book_id: book.id, overall_progress: 0 },
    ...
    if (error.response && error.response.status === 409) {
       ...
       console.error(error)
       showToast('Не вдалося додати книгу', 'error')
    }
  } finally {
...
const removeFromLibrary = async (book) => {
  if (!confirm('Видалити книгу з бібліотеки? Прогрес буде втрачено.')) return
  
  book.isProcessing = true
  try {
    await axios.delete(`${API_URL}/reader/${book.id}`, { withCredentials: true }) 

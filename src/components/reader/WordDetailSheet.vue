<template>
  <Transition name="sheet">
    <div v-if="isOpen" class="fixed inset-0 z-50 flex items-end justify-center" @click.self="close">
      <div class="absolute inset-0 bg-black/40 backdrop-blur-sm" @click="close"></div>

      <div
        class="relative w-full max-w-2xl bg-white rounded-t-3xl shadow-2xl max-h-[85vh] flex flex-col animate-slide-up"
      >
        ...
        <template v-else>
          ...
          <div class="flex-1 overflow-y-auto px-6 py-4 space-y-6">
            ...
            <div
              v-if="word.additionalTranslations && word.additionalTranslations.length > 0"
              class="space-y-3"
            >
              <h3 class="text-lg font-semibold text-black">Інші варіанти</h3>
              <div class="flex flex-wrap gap-2">
                <span
                  v-for="(translation, index) in word.additionalTranslations"
                  :key="index"
          ...
          <div class="p-4 border-t border-gray-200 bg-white">
            ...
            <p v-if="errorMessage" class="text-sm text-red-600 mt-3 text-center">
              {{ errorMessage }}
            </p>
          </div>
        </template>
...
const addToDictionary = async () => {
  ...
  try {
    await api.post('/learning/', {
      ...
      context: props.word.contextSentence || ''
    })

    emit('add-to-dictionary', props.word)
    close()
  } catch (error) {
    errorMessage.value = getErrorMessage(error, 'Не вдалося додати слово. Спробуйте ще раз.')
  } finally {
